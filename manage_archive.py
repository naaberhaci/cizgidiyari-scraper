...
def detect_missing(path, replace_missing):
    # cd to the directory
    os.chdir(path)
    last_no = 0
    missing = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("path", help="Path to the directory", nargs="?", default=".")
    ...
    parser.add_argument("--detect-missing", help="List missing files", action="store_true")
    parser.add_argument("--replace-missing", help="Detect and replace missing files", action="store_true")
    ...
    yes = args.yes
    detect_missing_flag = args.detect_missing
    replace_missing_flag = args.replace_missing

    if not os.path.exists(path):
        print("Path does not exist:", path)
        sys.exit(1)

    if detect_missing_flag or replace_missing_flag:
